...
<div class="card mb-4">
    <div class="card-body">
        <form action="{{ route('admin.transaksis.update', $transaksi->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="id_pesanan" id="id_pesanan" class="form-control @error('id_pesanan') is-invalid @enderror" required>
                        ...
                        @foreach($pesanans as $pesanan)
                            <option value="{{ $pesanan->id }}" {{ old('id_pesanan', $transaksi->id_pesanan) == $pesanan->id ? 'selected' : '' }}>#{{ $pesanan->id }} - {{ $pesanan->user->name ?? '-' }}</option>
                        @endforeach
                    </select>
                    @error('id_pesanan')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" name="jumlah_bayar" id="jumlah_bayar" class="form-control @error('jumlah_bayar') is-invalid @enderror" value="{{ old('jumlah_bayar', $transaksi->jumlah_bayar) }}" min="0" placeholder="0" required>
                    </div>
                    @error('jumlah_bayar')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6 mb-3">
                    ...
                    @if($transaksi->bukti_transfer)
                        <div class="mb-2">
                            <a href="{{ asset('storage/' . $transaksi->bukti_transfer) }}" target="_blank" class="btn btn-sm btn-info">
                                <i class="fas fa-file"></i> Lihat Bukti
                            </a>
                        </div>
                    @endif
                    <input type="file" name="bukti_transfer" id="bukti_transfer" class="form-control @error('bukti_transfer') is-invalid @enderror" accept="image/*">
